...
artiCommentRouter.get('', async (req, res, next) => {
  const comment = await prisma.ArtiComment.findMany({
    take: 3,
    orderBy: {
      createdAt: 'asc'
...
artiCommentRouter.get('/:cursor', async (req, res, next) => {
...
artiCommentRouter.post('', async (req, res, next) => {
...
artiCommentRouter.patch('/:id', async (req, res, next) =>{
  const { id } = req.params;
...
artiCommentRouter.delete('/:id', async (req, res, next) => {
  const { id } = req.params;
